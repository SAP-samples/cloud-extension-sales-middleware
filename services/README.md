...
As value for Host Name field should be the Kyma host URL (take <i>https://console.XXc2fb6.kyma.shoot.live.k8s-hana.ondemand.com</i>),
...
- For <code>System Instance ID</code>, select system which was already created with the name <code>US_SALES_MIDDLEWARE_DEMO</code>, if you have chosen to give a
  different name please select that name.
- Click the button <code>Next</code> from the bottom right.
...
const baseURL = __salesCloudGatewayUrl();
```
