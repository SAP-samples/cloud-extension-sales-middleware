...
class VisitsCacheClient extends BaseCacheClient {
  constructor(cache) {
    super(cache)
    this.cache = cache;
  }
  ...
  _getVisitIdsBy(visitsSetKey) {
    if (!this.cache) {
      return new Promise((resolve, reject) => {
        return resolve({ cacheHit: false, result: [] });
      })
    }
    return this
      .cache
  ...
  deleteFromCache(visitId) {
    ...
    return this
      ...
      .then(cachedVisit => {
        ...
        const userVisitSetKey = this._buildVisitsSetKey(visit.Owner, visit.StartDateTime);
  ...
  addNewVisitInCache(visitODataResponse) {
    ...
    return new Promise((resolve, reject) => {
      const userVisitDateKey = this._buildVisitsSetKey(visitODataResponse.Owner, visitODataResponse.StartDateTime);
      this
        .cache
  ...
  cacheVisit(visit) {
    ...
    const visitCacheKey = this._buildVisitKey(visit.ObjectID);
    this
      .cache
  ...
  _buildVisitsSetKey(userId, dateAsString) {
    return this._buildUserVisitSetKeyWithFormat(userId, dateAsString);
  }

  _buildUserVisitSetKeyWithFormat(userId, dateAsString) {
    return buildCacheKeyFrom({
      userId,
      date: dateAsString,
      type: 'userVisits',
...
module.exports = new VisitsCacheClient(cache.cacheRedis);
