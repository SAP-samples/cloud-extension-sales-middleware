...
const { findKeys, splitKeyIntoComponents } = require('../helpers/redis');
const {
...
const logger = require('../logger/console')("VisitsCache");
...
class VisitsCache {
  ...
  addNewVisitInCache(visitODataResponse) {
    visitODataResponse = sanitize(visitODataResponse)
  ...
  updateVisitInCache(updatedVisit) {
    updatedVisit = sanitize(updatedVisit)
    return this
...
module.exports = {
  factory: (cache) => new VisitsCache(cache),
};
